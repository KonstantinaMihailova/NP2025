...
interface Task {
    ...
    static final Comparator<Task> BY_ORDER_COMPARATOR=Comparator.comparing(Task::getOrder);
    int getOrder();
}
    ...
    public static <T extends Task> TaskScheduler<T> getOrdered() {
        ...
        return new TaskScheduler<T>() {
            ...
            public List<T> schedule(T[] tasks) {
                return Arrays.stream(tasks).sorted(Task.BY_ORDER_COMPARATOR).collect(Collectors.toList());
            }
        };
